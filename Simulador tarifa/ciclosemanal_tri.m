function [x1 y1 z1 x2 y2 z2 x3 y3 z3]=ciclosemanal_tri(table_tariff,kWh,kVA,kVA_i,kWh_j,trf_j,cd_ib,cd_ie)
            ...
            for cd_i=cd_ib:cd_ie  
                ...
                if ciclo_semanal(cd_i,2)==1
                    ...
                    if ciclo_semanal(cd_i,3)==3
                        ...
                        if ciclo_semanal(cd_i,4)>=ciclo_semanal(cd_i,5)
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4) || kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                                x1=x1+kWh(kWh_i,kWh_j)*table_tariff(kVA_i,trf_j+3);
                            end
                        else
                    ...
                    elseif ciclo_semanal(cd_i,3)==2
                        ...
                        if ciclo_semanal(cd_i,4)>=ciclo_semanal(cd_i,5)
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4) || kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                                y1=y1+kWh(kWh_i,kWh_j)*table_tariff(kVA_i,trf_j+2);
                            end
                        else
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4)
                                if kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                    ...
                    elseif ciclo_semanal(cd_i,3)==1
                        ...
                        if ciclo_semanal(cd_i,4)>=ciclo_semanal(cd_i,5)
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4) || kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                                z1=z1+kWh(kWh_i,kWh_j)*table_tariff(kVA_i,trf_j+1);
                            end
                        else
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4)
                                if kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
            ...
            for cd_i=cd_ib:cd_ie            
                ...
                if ciclo_semanal(cd_i,2)==2  
                    ...
                    if ciclo_semanal(cd_i,3)==3
                        ...
                        if ciclo_semanal(cd_i,4)>=ciclo_semanal(cd_i,5)
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4) || kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                                x2=x2+kWh(kWh_i,kWh_j)*table_tariff(kVA_i,trf_j+3);
                            end
                        else
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4)
                                if kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                    ...
                    elseif ciclo_semanal(cd_i,3)==2
                        ...
                        if ciclo_semanal(cd_i,4)>=ciclo_semanal(cd_i,5)
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4) || kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                                y2=y2+kWh(kWh_i,kWh_j)*table_tariff(kVA_i,trf_j+2);
                            end
                        else
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4)
                                if kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                    ...
                    elseif ciclo_semanal(cd_i,3)==1
                        ...
                        if ciclo_semanal(cd_i,4)>=ciclo_semanal(cd_i,5)
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4) || kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                                z2=z2+kWh(kWh_i,kWh_j)*table_tariff(kVA_i,trf_j+1);
                            end
                        else
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4)
                                if kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
        ...
        for cd_i=cd_ib:cd_ie      
            ...
            if ciclo_semanal(cd_i,2)==3   
                ...
                if ciclo_semanal(cd_i,3)==3
                    ...
                    if ciclo_semanal(cd_i,4)>=ciclo_semanal(cd_i,5)
                        if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4) || kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                            x3=x3+kWh(kWh_i,kWh_j)*table_tariff(kVA_i,trf_j+3);
                        end
                    else
                        if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4)
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                ...
                elseif ciclo_semanal(cd_i,3)==2
                    ...
                    if ciclo_semanal(cd_i,4)>=ciclo_semanal(cd_i,5)
                        if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4) || kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                            y3=y3+kWh(kWh_i,kWh_j)*table_tariff(kVA_i,trf_j+2);
                        end
                    else
                        if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4)
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                ...
                elseif ciclo_semanal(cd_i,3)==3
                    ...
                    if ciclo_semanal(cd_i,4)>=ciclo_semanal(cd_i,5)
                        if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4) || kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
                            z3=z3+kWh(kWh_i,kWh_j)*table_tariff(kVA_i,trf_j+1);
                        end
                    else
                        if kWh(kWh_i,1)+kWh(kWh_i,2)/60>ciclo_semanal(cd_i,4)
                            if kWh(kWh_i,1)+kWh(kWh_i,2)/60<=ciclo_semanal(cd_i,5)
